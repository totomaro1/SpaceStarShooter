...
namespace Ninez.Board
{
    public class Block
    {
        ...
        public bool DoEvaluation(BoardEnumerator boardEnumerator, int nRow, int nCol)
        {
            ...
            //3. 클리어 조건에 아직 도달하지 않는 경우 NORMAL 상태로 복귀
            status = BlockStatus.NORMAL;
            match = MatchType.NONE;
            matchCount = 0;
         ...
         */
        public void UpdateBlockStatusMatched(MatchType matchType, bool bHorz, bool bAccumulate = true)
        {
            ...
            if (match == MatchType.NONE)
            {
                ...
                //Debug.Log($"matchTypeSrc : {match} , bHorz : {bHorz}");
            }
            else
            {
                this.match = bAccumulate ? match.Add(matchType) : matchType; //match + matchType
                //Debug.Log($"matchTypeSrc : {match} , matchTypeTarget : {matchType}, bHorz : {bHorz}");
            }
            ...
            //블럭 파괴 방법 도입
            //4match 가로
            if (match == MatchType.FOUR && bHorz == true) questType = BlockQuestType.CLEAR_HORZ;
            //4match 세로
            if (match == MatchType.FOUR && bHorz == false) questType = BlockQuestType.CLEAR_VERT;
            //5match
            if (match == MatchType.FIVE) questType = BlockQuestType.CLEAR_LAZER;

            //3x3 , 3x4 , 3x5 , 4x4 , 4x5 match CIRCLE 부여
            if (MatchType.THREE_THREE <= match) questType = BlockQuestType.CLEAR_CIRCLE;

            //4x5 , 5x5 match ALLCLEAR 부여
            if (MatchType.FOUR_FIVE <= match) questType = BlockQuestType.CLEAR_ALL;
        }
